...
class TransactionsPage {
  ...
  constructor( element ) {
    if (!element) {
      throw new Error('Передан пустой элемен');
    }
  ...
  registerEvents() {
    this.element.addEventListener('click', (e) => {
      if(e.target.contains('remove-account')) {
        this.removeAccount();
      } else if (e.target.contains('transaction__remove')) {
  ...
  getTransactionHTML(item){
    const dateItem = this.formatDate(item.created_at);
